...
export const getAllRegions = async (): Promise<Region[] | AxiosError | undefined> => {
    return axios.get(ENDPOINT + '/all?sort[]=name').then((response: AxiosResponse<RegionList>) => response.data.data).catch((error) => {
        return error;
    });
}
...
export const getRegions = (query ?: String): Promise<RegionPaginate> => {
    ...
    return axios.get(url).then((response: AxiosResponse<RegionPaginate>) => response.data);
}
