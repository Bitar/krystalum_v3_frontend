...
export const getAllCities = async (): Promise<City[] | AxiosError | undefined> => {
    return axios.get(ENDPOINT + '/all?sort[]=name').then((response: AxiosResponse<CityList>) => response.data.data).catch((error) => {
        return error;
    });
}
...
export const getCities = (query?: String): Promise<CityPaginate> => {
    ...
    return axios.get(url).then((response: AxiosResponse<CityPaginate>) => response.data);
}
...
export const updateCity = async (id: number, city: any): Promise<City | AxiosError | undefined> => {
    let formData = createFormData(city);
