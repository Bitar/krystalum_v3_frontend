...
import {PublicationEditOptions, PublicationOptions, PublisherOptions} from '../../models/supply/Options';
...
export const getPublisherOptions = async (): Promise<PublisherOptions | AxiosError | undefined> => {
    ...
    return axios.get(url).then((response: AxiosResponse<PublisherOptions>) => response.data).catch((error) => {
        return error;
    });
}
