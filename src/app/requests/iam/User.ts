...
export const getAllUsers = async (query?: string): Promise<UserList | AxiosError | undefined> => {
    return axios.get(ENDPOINT + '/all?sort[]=name' + (query ? '&' + query : '')).then((response: AxiosResponse<UserList>) => response.data).catch((error) => {
        return error;
    });
}
