                ...
                <div className='d-flex flex-wrap fs-6 mb-4 pe-2'>
                  {publisher?.info?.email && (
                    <OverlayTrigger placement='top' overlay={<Tooltip>Email address</Tooltip>}>
                      <a
                        href={`mailto:${publisher?.info?.email}`}
                  ...
                  {publisher?.info?.hq_country && (
                    <OverlayTrigger placement='top' overlay={<Tooltip>HQ country</Tooltip>}>
                      <div className='d-flex align-items-center text-gray-400 me-5 mb-2'>
                        <KTSVG
                          path='/media/icons/duotune/maps/map008.svg'
                          className='svg-icon-4 me-1'
                        />
                        {publisher?.info?.hq_country?.name}
                      </div>
                    </OverlayTrigger>
                  )}

                  {publisher?.integration_date && (
                    <OverlayTrigger placement='top' overlay={<Tooltip>Integration date</Tooltip>}>
                      <div className='d-flex align-items-center text-gray-400 me-5 mb-2'>
                        <KTSVG
                          path='/media/icons/duotune/general/gen014.svg'
                          className='svg-icon-4 me-1'
                        />
                        {formatDateToMonthDayYear(publisher?.integration_date)}
                      </div>
                    </OverlayTrigger>
                  )}
                </div>
