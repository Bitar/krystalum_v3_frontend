...
const PublisherAccountManagersColumns: ReadonlyArray<Column<PublisherAccountManager>> = [
  ...
  {
    Header: (props) => (
      <CustomHeader tableProps={props} title='Assignment date' className='min-w-125px' />
    ),
    id: 'assignment-date',
    Cell: ({...props}) => (
      <TextCell
        text={
          props.data[props.row.index].assignment_date
            ? toDateTimeString(new Date(props.data[props.row.index].assignment_date))
            : ''
        }
      />
    ),
