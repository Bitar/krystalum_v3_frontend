...
export const publicationSchema = (isEdit: boolean) => {
  let schema = {
    ...
    publisher_id: Yup.number().min(1, 'publisher is a required field').required(),
    ...
    live_date: Yup.date().typeError('enter a valid date').required('date is a required field'),
    has_hi10: Yup.boolean().required(),
    hi10_to_display: Yup.mixed().when('has_hi10', (hasHi10, hi10ToVideo) =>
      hasHi10 === 0 && hi10ToVideo === 0 ? Yup.boolean().required() : Yup.boolean().notRequired()
    ),
    hi10_to_video: Yup.mixed().when('has_hi10', (hasHi10, hi10ToDisplay) =>
      hasHi10 === 1 && hi10ToDisplay === 1 ? Yup.boolean().required() : Yup.boolean().notRequired()
    ),
    types: Yup.array().min(1, 'select at least one publication type').required(),
    url: Yup.string().when('types', {
      ...
      then: Yup.string()
        ...
        .test(
          'trailing-slash',
          'URL should end with a forward slash',
    ...
    android_bundle_id: Yup.string().when('types', {
      ...
      then: Yup.string().required(
        'android bundle id is required when the publication type is android application'
      ),
      otherwise: Yup.string().notRequired(),
    }),
    android_version: Yup.string().when('types', {
      is: (types: string[]) => types.includes(PublicationTypeEnum.ANDROID_APPLICATION),
      then: Yup.string().required(
        'android version is required when the publication type is android application'
      ),
      otherwise: Yup.string().notRequired(),
    }),
    android_application_type: Yup.string().when('types', {
      is: (types: string[]) => types.includes(PublicationTypeEnum.ANDROID_APPLICATION),
      then: Yup.string().required(
        'android application type is required when the publication type is android application'
      ),
      otherwise: Yup.string().notRequired(),
    }),
    ...
    ios_bundle_id: Yup.string().when('types', {
      is: (types: string[]) => types.includes(PublicationTypeEnum.IOS_APPLICATION),
      then: Yup.string().required(
        'ios bundle id is required when the publication type is ios application'
      ),
      otherwise: Yup.string().notRequired(),
    }),
    ios_version: Yup.string().when('types', {
      is: (types: string[]) => types.includes(PublicationTypeEnum.IOS_APPLICATION),
      then: Yup.string().required(
        'ios version is required when the publication type is ios application'
      ),
      otherwise: Yup.string().notRequired(),
    }),
    ios_application_type: Yup.string().when('types', {
      is: (types: string[]) => types.includes(PublicationTypeEnum.IOS_APPLICATION),
      then: Yup.string().required(
        'ios application type is required when the publication type is ios application'
      ),
      otherwise: Yup.string().notRequired(),
    }),
    ...
    revenue_value: Yup.mixed().when('revenue_type', (revenueType) =>
      revenueType === RevenueTypeEnum.REVENUE_SHARE
        ? Yup.number()
            .min(1, 'revenue share must be greater than 0')
            .max(100, 'revenue share must be less than or equal to 100')
            .required()
        : revenueType === RevenueTypeEnum.COMMITMENT
        ? Yup.string().required()
