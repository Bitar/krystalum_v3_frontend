...
const PublicationCampaignRestrictionsColumns: ReadonlyArray<Column<PublicationCampaignRestriction>> = [
    {
        ...
        Cell: ({...props}) => {
            return (props.data[props.row.index].geos.length > 0 ?
                <BadgeCell status={props.data[props.row.index].geos[0].geoType.name}
                           color={props.data[props.row.index].geos[0].geoType.id === GeoTypeEnum.REGION ? 'light-primary' : 'light-info'}
                           align="left"/> : <TextCell text={'N/A'}></TextCell>)
        },
    },
    {
        ...
        id: 'geos',
        Cell: ({...props}) => <TextCell
            text={props.data[props.row.index].geos.length > 0 ? props.data[props.row.index].geos.map((publicationCampaignRestrictionGeo: PublicationCampaignRestrictionGeo) => publicationCampaignRestrictionGeo.geo.name).join(', ')
                : 'N/A'}/>,
    },
    {
        ...
        id: 'formats',
        Cell: ({...props}) => <TextCell
            text={props.data[props.row.index].formats?.length > 0 ? truncateText(props.data[props.row.index].formats?.map((format: Format) => format.name).join(', '))
                : 'N/A'}/>,
    },
    {
        Header: (props) => <CustomHeader tableProps={props} title="Campaign types" className="min-w-125px"/>,
        id: 'campaign-types',
        Cell: ({...props}) => <TextCell
            text={props.data[props.row.index].campaignTypes?.length > 0 ? truncateText(props.data[props.row.index].campaignTypes?.map((campaignType: CampaignType) => campaignType.name).join(', '))
                : 'N/A'}/>,
    },
    {
        Header: (props) => <CustomHeader tableProps={props} title="Website pages" className="min-w-125px"/>,
        id: 'website-pages',
        Cell: ({...props}) => <TextCell
            text={props.data[props.row.index].websitePages?.length > 0 ? truncateText(props.data[props.row.index].websitePages?.map((websitePage: WebsitePage) => websitePage.name).join(', '))
                : 'N/A'}/>,
    },
    {
        Header: (props) => <CustomHeader tableProps={props} title="Campaign restriction requirements"
