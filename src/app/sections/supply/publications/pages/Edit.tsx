...
import PublicationCampaignRestriction from './edit/restrictions/Restriction'
...
const PublicationEdit: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (publication) {
      krysApp.setPageTitle(
        generatePageTitle(Sections.SUPPLY_PUBLICATIONS, PageTypes.EDIT, publication.name)
      )
    }
  ...
  const settingsNav = [
    ...
    },
    {
      title: 'Ad Servers',
      description:
        'Quickly access and update the list of ad servers associated with the publication',
                  ...
                  <Nav.Item key={`settings-nav-${index}`} className='mb-5'>
                    <Nav.Link className='settings-nav-item' eventKey={`settingsNav-${index}`}>
                      ...
                      <div className='settings-nav-label'>
                        <span className='settings-nav-title text-gray-800'>{settings.title}</span>
